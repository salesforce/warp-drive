...
def ComputeDistance(loc_x_arr,
                    loc_y_arr,
                    kThisAgentId1,
                    kThisAgentId2,
                    kEnvId,
                    kNumAgents):
    return math.sqrt(((loc_x_arr[kEnvId, kThisAgentId1] - loc_x_arr[kEnvId, kThisAgentId2]) ** 2) + \
                     ((loc_y_arr[kEnvId, kThisAgentId1] - loc_y_arr[kEnvId, kThisAgentId2]) ** 2))

# Device helper function to generate observation
@numba_driver.jit(device=True)
def CudaTagContinuousGenerateObservation(loc_x_arr,
                                         loc_y_arr,
                                         speed_arr,
                                         direction_arr,
                                         acceleration_arr,
    ...
    if kThisAgentId < kNumAgents:
        if kUseFullObservation:
            ...
            if still_in_the_game_arr[kEnvId, kThisAgentId]:
                ...
                for other_agent_id in range(kNumAgents):
                    if not other_agent_id == kThisAgentId:
                        obs_arr[kEnvId, kThisAgentId, 0 * (kNumAgents - 1) + index] = \
                            float(loc_x_arr[kEnvId, other_agent_id] - loc_x_arr[kEnvId, kThisAgentId]) / (math.sqrt(2.0) * kGridLength)
                        obs_arr[kEnvId, kThisAgentId, 1 * (kNumAgents - 1) + index] = \
                            float(loc_y_arr[kEnvId, other_agent_id] - loc_y_arr[kEnvId, kThisAgentId]) / (math.sqrt(2.0) * kGridLength)
                        obs_arr[kEnvId, kThisAgentId, 2 * (kNumAgents - 1) + index] = \
                            float(speed_arr[kEnvId, other_agent_id] - speed_arr[kEnvId, kThisAgentId]) / (kMaxSpeed + kEpsilon)
                        obs_arr[kEnvId, kThisAgentId, 3 * (kNumAgents - 1) + index] = \
                            float(acceleration_arr[kEnvId, other_agent_id] - acceleration_arr[kEnvId, kThisAgentId]) / (kMaxSpeed + kEpsilon)
                        obs_arr[kEnvId, kThisAgentId, 4 * (kNumAgents - 1) + index] = \
                            float(direction_arr[kEnvId, other_agent_id] - direction_arr[kEnvId, kThisAgentId]) / kTwoPi
                        index += 1

                obs_arr[kEnvId, kThisAgentId, num_features * (kNumAgents - 1)] = float(env_timestep_arr[kEnvId]) / kEpisodeLength
        else:
            ...
            if still_in_the_game_arr[kEnvId, kThisAgentId]:
                ...
                for other_agent_id in range(kNumAgents):
                    if not other_agent_id == kThisAgentId and still_in_the_game_arr[kEnvId, other_agent_id]:
                        neighbor_ids_sorted_by_distance_arr[kEnvId, kThisAgentId, num_valid_other_agents] = other_agent_id
                        num_valid_other_agents += 1

                # First, find distance to all the valid agents
                for idx in range(num_valid_other_agents):
                    neighbor_distances_arr[kEnvId, kThisAgentId, idx] = ComputeDistance(loc_x_arr,
                                                                               loc_y_arr,
                                                                               kThisAgentId,
                ...
                for idx in range(min(num_valid_other_agents, kNumOtherAgentsObserved)):
                    ...
                    obs_arr[kEnvId, kThisAgentId, 0 * kNumOtherAgentsObserved + idx] = \
                        float(loc_x_arr[kEnvId, kOtherAgentId] - loc_x_arr[kEnvId, kThisAgentId]) / (math.sqrt(2.0) * kGridLength)
                    obs_arr[kEnvId, kThisAgentId, 1 * kNumOtherAgentsObserved + idx] = \
                        float(loc_y_arr[kEnvId, kOtherAgentId] - loc_y_arr[kEnvId, kThisAgentId]) / (math.sqrt(2.0) * kGridLength)
                    obs_arr[kEnvId, kThisAgentId, 2 * kNumOtherAgentsObserved + idx] = \
                        float(speed_arr[kEnvId, kOtherAgentId] - speed_arr[kEnvId, kThisAgentId]) / (kMaxSpeed + kEpsilon)
                    obs_arr[kEnvId, kThisAgentId, 3 * kNumOtherAgentsObserved + idx] = \
                        float(acceleration_arr[kEnvId, kOtherAgentId] - acceleration_arr[kEnvId, kThisAgentId]) / (kMaxSpeed + kEpsilon)
                    obs_arr[kEnvId, kThisAgentId, 4 * kNumOtherAgentsObserved + idx] = \
                        float(direction_arr[kEnvId, kOtherAgentId] - direction_arr[kEnvId, kThisAgentId]) / kTwoPi
                    obs_arr[kEnvId, kThisAgentId, 5 * kNumOtherAgentsObserved + idx] = \
                        agent_types_arr[kOtherAgentId]
                    obs_arr[kEnvId, kThisAgentId, 6 * kNumOtherAgentsObserved + idx] = \
...
@numba_driver.jit(device=True)
def CudaTagContinuousComputeReward(rewards_arr,
                                    loc_x_arr,
                                    loc_y_arr,
                                    kGridLength,
    ...
    if kThisAgentId < kNumAgents:
        ...
        if still_in_the_game_arr[kEnvId, kThisAgentId]:
            ...
            rewards_arr[kEnvId, kThisAgentId] += edge_hit_reward_penalty[kEnvId, kThisAgentId]
        ...
        if is_runner and still_in_the_game_arr[kEnvId, kThisAgentId]:
            for other_agent_id in range(kNumAgents):
                ...
                if is_tagger:
                    dist = ComputeDistance(loc_x_arr,
                                           loc_y_arr,
                                           kThisAgentId,
                                           other_agent_id,
                                           kEnvId,
...
@numba_driver.jit
def NumbaTagContinuousStep(loc_x_arr,
                           loc_y_arr,
                           speed_arr,
                           direction_arr,
                           acceleration_arr,
    ...
    if (kThisAgentId < kNumAgents):
        ...
        direction_arr[kEnvId, kThisAgentId] = ((direction_arr[kEnvId, kThisAgentId] + delta_turn) % kTwoPi) * \
                                            still_in_the_game_arr[kEnvId, kThisAgentId]
        
        if (direction_arr[kEnvId, kThisAgentId] < 0):
            direction_arr[kEnvId, kThisAgentId] = kTwoPi + direction_arr[kEnvId, kThisAgentId]

        # Speed clipping
        speed_arr[kEnvId, kThisAgentId] = min(kMaxSpeed * skill_levels_arr[kThisAgentId],
                                       max(0.0,
                                           speed_arr[kEnvId, kThisAgentId] + acceleration_arr[kEnvId, kThisAgentId])) * \
                                   still_in_the_game_arr[kEnvId, kThisAgentId]

        # Reset acceleration to 0 when speed becomes 0 or kMaxSpeed (multiplied by skill levels)
        if (speed_arr[kEnvId, kThisAgentId] <= 0.0 or
            speed_arr[kEnvId, kThisAgentId] >= (kMaxSpeed * skill_levels_arr[kThisAgentId])):
            acceleration_arr[kEnvId, kThisAgentId] = 0.0

        loc_x_arr[kEnvId, kThisAgentId] += speed_arr[kEnvId, kThisAgentId] * math.cos(direction_arr[kEnvId, kThisAgentId])
        loc_y_arr[kEnvId, kThisAgentId] += speed_arr[kEnvId, kThisAgentId] * math.sin(direction_arr[kEnvId, kThisAgentId])

        # Crossing the edge
        has_crossed_edge = loc_x_arr[kEnvId, kThisAgentId] < 0 or \
                           loc_x_arr[kEnvId, kThisAgentId] > kGridLength or \
                           loc_y_arr[kEnvId, kThisAgentId] < 0 or \
    ...
    CudaTagContinuousGenerateObservation(loc_x_arr,
                                         loc_y_arr,
                                         speed_arr,
                                         direction_arr,
                                         acceleration_arr,
    ...
    # Compute reward                 -
    # --------------------------------
    CudaTagContinuousComputeReward(rewards_arr,
                                   loc_x_arr,
                                   loc_y_arr,
                                   kGridLength,
                                   edge_hit_reward_penalty,
